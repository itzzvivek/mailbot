...
def test_or_logic_across_multiple_filters():
    message = {"label_ids": {"CATEGORY_UPDATES"}}
    # Channel wants primary OR updates -- message only has updates, should still match.
...
# ---- _decode (MIME header decoding) -----------------------------------------

def test_decode_plain_ascii_passthrough():
    assert il._decode("Hello world") == "Hello world"


def test_decode_mime_encoded_subject():
    # Base64-encoded "Hello world" as a MIME encoded-word.
    encoded = "=?UTF-8?B?SGVsbG8gd29ybGQ=?="
    assert il._decode(encoded) == "Hello world"


def test_decode_does_not_raise_on_garbage():
    result = il._decode("not =?broken?= header")
